...
const EnhancedGallerySection = () => {
  ...
  return <section id="gallery" className="py-20 px-4 min-h-screen flex items-center">
      <div className="container mx-auto">
        ...
        {/* Circular Gallery */}
        <div ref={containerRef} className="relative w-full h-[600px] flex items-center justify-center" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
          {/* Particle Effects */}
          {particles.map(particle => <div key={particle.id} className="absolute w-1 h-1 bg-primary rounded-full pointer-events-none" style={{
          left: '50%',
          top: '50%',
          ...
          return <div key={conquest.id} className={`absolute w-64 h-80 transition-all duration-500 cursor-pointer group ${isHovered ? 'z-20' : 'z-10'}`} style={{
            transform: `translate(${position.x}px, ${position.y}px) ${isHovered ? 'scale(1.1) rotateY(180deg)' : 'scale(1)'}`,
            transformStyle: 'preserve-3d'
          }} onMouseEnter={() => handleCardHover(conquest.id)} onMouseLeave={() => setHoveredCard(null)}>
                {/* Card Front */}
                <div className={`absolute inset-0 glass-card rounded-lg overflow-hidden backface-hidden ${isHovered ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`} style={{
              backfaceVisibility: 'hidden'
            }}>
                  <div className="relative h-full">
                    <img src={conquest.image} alt={conquest.title} className="w-full h-full object-cover" />
                ...
                {/* Card Back */}
                <div className={`absolute inset-0 glass-card rounded-lg p-6 flex flex-col justify-center ${isHovered ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`} style={{
              backfaceVisibility: 'hidden',
