    ...
    <section className="py-20 px-4 bg-bg-secondary">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="relative">
          ...
          <button
            onClick={prevCard}
            className="absolute left-4 top-1/2 transform -translate-y-1/2 z-10 p-3 bg-bg-dark/80 backdrop-blur-sm rounded-full border border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-300 neon-glow hover-scale"
          >
          ...
          <button
            onClick={nextCard}
            className="absolute right-4 top-1/2 transform -translate-y-1/2 z-10 p-3 bg-bg-dark/80 backdrop-blur-sm rounded-full border border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-300 neon-glow hover-scale"
          >
          ...
          >
            ...
            >
              {journeyCards.map((card, index) => (
                ...
                >
                  <div className={`glass-card p-8 md:p-12 rounded-lg transition-all duration-500 hover-glow ${
                    index === currentCard ? 'scale-105 shadow-[0_0_30px_hsl(var(--primary))]' : ''
                  }`}>
          ...
          <div className="mt-6 w-full bg-bg-dark rounded-full h-1">
            <div 
              ...
              style={{ width: `${((currentCard + 1) / journeyCards.length) * 100}%` }}
            />
          </div>
        </div>
